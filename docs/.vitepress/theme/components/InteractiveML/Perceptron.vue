<template>
  ...
  <div class="demo-container">
    ...
    <div class="controls">
      <div class="controls-l">
        ...
        <!--div class="toggle-group">
          <input type="checkbox" id="ood-toggle" v-model="showOOD">
          <label for="ood-toggle">显示分布外样本 (OOD)</label>
        </div-->
      </div>
    ...
    <div class="explanation">
      ...
      <p v-if="showOOD" class="ood-warning">
        ⚠️ OOD 样本已超出训练分布，分类结果可能完全错误！
      </p>
    </div>
  </div>
</template>
...
// 初始参数（对应你公式中的 W^T X + b）
const w1 = ref(-0.3);
const w2 = ref(0.5);
const b = ref(-0.1);
const showOOD = ref(false);
...
const draw = () => {
  ...
  // 绘制训练样本（分布内）
  const inDistSamples = [
    { x: 150, y: 150, label: 1 },  // 正类
    { x: 250, y: 50, label: -1 }  // 负类
  ];
  inDistSamples.forEach(sample => {
    const pred = w1.value * (sample.x - c.width / 2) + w2.value * (sample.y - c.height / 2) + b.value * 100;
    const color = pred >= 0 ? '#4cc9f0' : '#f72585';

    ctx.value.beginPath();
    ctx.value.arc(sample.x, sample.y, 10, 0, Math.PI * 2);
  ...
  // 绘制分布外样本（可选）
  if (showOOD.value) {
    const oodSample = { x: 150, y: 50 }; // 明显超出训练范围
    const pred = w1.value * (oodSample.x - c.width / 2) + w2.value * (oodSample.y - c.height / 2) + b.value * 100;
    const color = pred >= 0 ? '#4cc9f0' : '#f72585';

    ctx.value.beginPath();
    ctx.value.arc(oodSample.x, oodSample.y, 15, 0, Math.PI * 2);
    ctx.value.fillStyle = color;
    ctx.value.fill();
    ctx.value.strokeStyle = '#e63946';
    ctx.value.lineWidth = 2;
